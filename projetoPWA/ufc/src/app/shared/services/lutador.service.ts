...
const apiUrl = 'https://127.0.0.1:3000/lutador';
...
export class LutadorService {
  ...
  getLutadores(): Observable<Lutador[]> {
    ...
    .pipe(tap(lutadores => console.log('leu os lutadores')),
  ...
  pesquisaLutadores(termo: string, categoria: string): Observable<Lutador[]> {
    ...
    return this.http.get<Lutador[]>(apiUrl + '/pesquisa?key=4ccc9336b467b9cf58051ea123493ef114eae029' + query)
    .pipe(tap(_ => console.log(`Encontrou lutador que começa com: "${termo}"`)),
  ...
  getLutador(id: number): Observable<Lutador> {
    const url = `${apiUrl}/${id}`;
    return this.http.get<Lutador>(url)
    .pipe(tap(_ => console.log(`leu o lutador id=${id}`)),
  ...
  getLutadoresAleatorios(categoriaPeso: any, sexo: any): Observable<Lutador[]> {
    ...
    return this.http.get<Lutador[]>(url)
    .pipe(catchError(this.handleError<Lutador[]>('getLutadoresAleatorios', []))
    );
  }

  addLutador (lutador): Observable<Lutador> {
    return this.http.post<Lutador>(apiUrl, lutador, httpOptions)
    .pipe(tap(_ => console.log(`adicionou o lutador com w/ id=${lutador._id}`)),
  ...
  updateLutador(id, lutador): Observable<any> {
    const url = `${apiUrl}/${id}`;
    return this.http.put(url, lutador, httpOptions)
    .pipe(tap(_ => console.log(`atualiza o lutador com id=${id}`)),
  ...
  deleteLutador (id): Observable<Lutador> {
    ...
    return this.http.delete<Lutador>(url, httpOptions)
    .pipe(tap(_ => console.log(`remove o lutador com id=${id}`)),
